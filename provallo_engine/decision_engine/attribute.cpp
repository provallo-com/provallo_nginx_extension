...
namespace provallo
{
  ...
  discrete_attribute::discrete_attribute (
      ...
      const vector<attribute_value> &attribute_values) :
      attribute_definition (name, tag), _values_map (attribute_values)
  {
  ...
  discrete_attribute::discrete_attribute (const attribute_definition *deserial) :
      attribute_definition (deserial)
  {
    // Check size
...
// Get new attribute definition
  ...
  attribute_information::attribute_information (
      ...
      const vector<pair<attribute_name, std::string> > &attributes) :
      _count (attributes.size ()), _type (attributes.size ())
  {
    ...
    //  Sanity check
    if (auto it = _name_map.find (target_name) != _name_map.end ())
      {
	        std::cout << "reverse_sanity success..." << it << std::endl;
      }
    ...
    if (_definition_map[_target_pos]->get_type ()
	== continous_attribute::_type ())
      print_error (
	...
	if (i != _target_pos && getType (i) != ignored_attribute::_type ())
	  {
	    ...
	    if (getType (i) == continous_attribute::_type ())
	      _groups.push (std::vector<attribute_tag> (1, i), CONE_BINARY,
			    CONTINUOUS);
  ...
  attribute_information::attribute_information (
      const attribute_information &right) :
      _tag_map (right._tag_map), _name_map (right._name_map), _target_pos (
	  right._target_pos), _definition_map (right._definition_map.size ()), _count (
	  right._count), _type (right._type), _groups (right._groups)
  {
  ...
  attribute_information::attribute_information (
      const attribute_information *deserial) :
	      _tag_map (deserial->_tag_map), _name_map (deserial->_name_map), _target_pos (
		  deserial->_target_pos), _definition_map (deserial->_definition_map.size ()), _count (
		      deserial->_count), _type (deserial->_type), _groups (deserial->_groups)
  ...
  attribute_information::~attribute_information ()
  {
    ...
    for (vector<attribute_definition*>::const_iterator it =
	    _definition_map.begin (); it != _definition_map.end (); ++it) 
          delete (*it);
