...
namespace provallo
{
  ...
  {
  public:
    explicit clustering_parameters(uint32_t k) : _k(k), _has_max_iter(false), _max_iter(), _has_min_delta(false), _min_delta(), _has_rand_seed(
                                                                                                                                    false),
                                                 _rand_seed()
    {
    }
               ...
               const clustering_parameters<T> &parameters)
  {
    ...
    uint64_t seed =
        parameters.has_random_seed() ? parameters.get_random_seed() : rand_device();
    ...
    do
    {
      ...
      means = calculate_means(data, clusters, old_means,
                              parameters.get_k());
      ++count;
    } while (means != old_means && means != old_old_means && !(parameters.has_max_iteration() && count == parameters.get_max_iteration()) && !(parameters.has_min_delta() && deltas_below_limit(deltas(old_means, means), parameters.get_min_delta())));
  ...
  {
    assert(
        points.size() == labels.size() && "Points and labels have different sizes");
  ...
  public:
    distributed_kmeans(const std::string &path) : _datapath(path)
    {
    }
    ...
    virtual ~distributed_kmeans() = default;
  };
