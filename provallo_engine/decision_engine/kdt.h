...
namespace provallo
{
  ...
  template<typename T>
    ...
    bpqueue<T>::best () const
    {
      return
	  empty () ?
	      std::numeric_limits<double>::infinity () : elems.begin ()->first;
    }
  ...
  template<typename T>
    ...
    bpqueue<T>::worst () const
    {
      return
	  empty () ?
	      std::numeric_limits<double>::infinity () : elems.rbegin ()->first;
    }
    ...
    private:
      struct node
      {
        ...
        node (const provallo::point<N> &_pt, int _level,
	      const elem_type &_value = elem_type ()) :
	    point (_pt), left (NULL), right (nullptr), level (_level), value (
		_value)
	    {
	    }
  ...
  template<std::size_t N, typename elem_type>
	...
	return NULL; // empty tree
      ...
      auto cmp = [axis]
      (const std::pair<point<N>, elem_type> &p1,
  ...
  template<std::size_t N, typename elem_type>
	...
	{ // recurse to the left side
	  return
	      currNode->left == NULL ? currNode : findNode (currNode->left, pt);
	}
      else
	{ // recurse to the right side
	  return
	      currNode->right == NULL ?
		  currNode : findNode (currNode->right, pt);
	}
    }
  ...
  template<std::size_t N, typename elem_type>
	...
	{ // insert pt with default elem_type value, and return reference to the new elem_type
	  ...
	  if (node == NULL)
	    ...
	    return
		(node->left != NULL && node->left->point == pt) ?
  ...
  template<std::size_t N, typename elem_type>
      ...
      else
	{
	  ...
	  || fabs (key[currLevel % N] - currpoint[currLevel % N])
	      < pQueue.worst ())
	{
