...
putrel:   n = *pl;
	  if (n < 0)
	    ...
	    {
linkrel:      b[pos] = n; /* Else link to rel chain, anchored at label. */
	      *pl = pos;
	    }
	  pos++;
	  break;
	...
	case DASM_LABEL_PC:
	  pl = D->pclabels + n;
	  CKPL(pc, PC);
putlabel: n = *pl; /* n > 0: Collapse rel chain and replace with label pos. */
	  while (n > 0)
	    {
	...
	case DASM_IMML8:
	case DASM_IMML12:
	  CK(n >= 0 ? ((n>>((ins>>5)&31)) == 0) : (((-n)>>((ins>>5)&31)) == 0),
	     RANGE_I);
	  b[pos++] = n;
	  break;
	case DASM_IMM12:
...
patchimml8:   cp[-1] |=
		  n >= 0 ?
		      (0x00800000 | (n & 0x0f) | ((n & 0xf0) << 4)) :
		      ((-n & 0x0f) | ((-n & 0xf0) << 4));
	      break;
	    case DASM_IMML12:
	    case DASM_IMMV8:
patchimml:    cp[-1] |= n >= 0 ? (0x00800000 | n) : (-n);
	      break;
	    default:
	      *cp++ = ins;
	      break;
