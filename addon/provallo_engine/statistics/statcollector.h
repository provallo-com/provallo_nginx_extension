...
namespace provallo
{
  ...
  public:
    ...
    stat_collector(const std::string &n) : _name(n), _parsed(false), _instances(1), _collected(false)
    {
  ...
  public:
    snmp4_collector() : _init(false), _ip_size(0), _tcp_size(0), _icmp_size(0), _udp_size(
                                                                                    0),
                        _udplite_size(0), stat_collector("SNMPv4")
    {
  ...
  stat_collector::putValues(InputIterator begin, InputIterator end)
  {
    ...
    // Sanity check
    assert(_instances[0]->size() == std::distance(begin, end)); //(end - begin));
    fields_instance *ptr(getNewDataPtr());
