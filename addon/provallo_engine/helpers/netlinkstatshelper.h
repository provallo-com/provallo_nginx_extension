...
#define NL_LINK_BUF_SIZE 16384
  ...
  public:
    ...
    explicit
    netlink_stats_helper (provallo::mini_dal *pDal,
			  const std::vector<provallo::stat_collector*> &rcoll,proc_collector* _c=nullptr) :
				_valid (false),_socket(0), _running (true), _pstats (nullptr),_collector(_c),_pDal(pDal),_collectors(rcoll)
    {
  ...
  protected:
    ...
    explicit
    netlink_stats_helper () :
	_valid (false), _running (true), _pstats (nullptr)
    {
