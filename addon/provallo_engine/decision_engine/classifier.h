...
namespace provallo
{
  ...
  public:
    ...
    classifier(const dataset &data, const parameter_base &parameters = none(),
               const std::random_device &ra = std::random_device(), split_method_factory *factory = nullptr) : _attributes_info(data.getattributes()), _split_factory(factory == nullptr ? new split_method_factory(data, _rand) : factory), _rand(), _name("classifier"), _data(data), _parameters(parameters)   
    {
    ...
    // A classifier should be constructed from a data set and a parameter object
    classifier( const classifier& other) : _attributes_info(other._attributes_info), _split_factory(other._split_factory),_parameters(other._parameters), _offsets(other._offsets), _rand(), _name(other._name),_data(other._data) 
    {
    } 
    classifier ( classifier&& other) : _attributes_info(other._attributes_info), _split_factory(other._split_factory), _offsets(other._offsets), _rand(), _name(other._name),_data(other._data), _parameters(other._parameters) 
    {
    }      

    // Construct classifier from buffer
    classifier(const classifier *deserial) : _attributes_info(deserial->_attributes_info), _split_factory(deserial->_split_factory), _offsets(deserial->_offsets), _rand(), _name(deserial->_name), _data(deserial->_data), _parameters(deserial->_parameters)
    {
    }   
  ...
  public:
    ...
    isolation_forest(int nthreads = 25, size_t ndim =3, size_t ntrees = 100,
  ...
  class iso_classifier : public classifier 
  {
    ...
    uint32_t _samples_per_tree;
    uint32_t _attributes_per_tree;
    uint32_t _attributes_per_sample;

    //tree count
    uint32_t _trees_count;
