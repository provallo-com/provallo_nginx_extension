...
#pragma omp parallel for schedule(static) num_threads(nthreads) shared(has_missing, prediction_data, imputer)
	...
	isolation_forest::isolation_forest(size_t ndim, size_t ntry,
									   provallo::CoefType coef_type,
...
#pragma omp parallel for if (nrows > 1) schedule(static) num_threads(nthreads) \
	...
	isolation_forest::isolation_forest(int nthreads, size_t ndim, size_t ntrees,
									   bool build_imputer) : nthreads(nthreads), ndim(ndim), ntrees(ntrees), build_imputer(build_imputer)
	{
		this->is_fitted = true;
	}
...
#pragma omp critical
	...
	//	Constructor
	iso_classifier::iso_classifier(const dataset &data) : _data(data), _attributes(data.getattributesNumber()), _trees(100), _subsample_size(256), _max_depth(10), classifier(data, none(),std::random_device(),nullptr )
	{
	...
	//Copy constructor
	iso_classifier::iso_classifier(const	iso_classifier & rhs): _data(rhs._data), _attributes(rhs._attributes), _trees(rhs._trees), _subsample_size(rhs._subsample_size), _max_depth(rhs._max_depth),   _isoforest(rhs._isoforest), classifier(rhs)		
	{
		validate_data();
		// create isoforest and train it
	...
	// Move constructor
	iso_classifier::iso_classifier(iso_classifier &&rhs) : _data(std::move(rhs._data)), _attributes(std::move(rhs._attributes)), _trees(std::move(rhs._trees)), _subsample_size(std::move(rhs._subsample_size)), _max_depth(std::move(rhs._max_depth)),  _isoforest(std::move(rhs._isoforest)), classifier(std::move((classifier&&)rhs))		
	{
	...
	void iso_classifier::validate_data()
	{
		...
		// Get number of trees
		_trees_count = (uint32_t)(std::log((double)samples_count) / std::log(2.0));
		...
		_samples_per_tree = (uint32_t)(samples_count / _trees_count);
		// Check number of samples per tree
		if (_samples_per_tree < 1)
		...
		_attributes_per_tree = (uint32_t)(attributes_count / 2);
		...
		_attributes_per_sample = (uint32_t)(samples_size / 2);
		// Check number of attributes per sample
		if (_attributes_per_sample < 1)
	...
	iso_classifier &iso_classifier::operator=(const iso_classifier &rhs)
	{
		if (this != &rhs)
		{
			...
			_trees_count = rhs._trees_count;
			_samples_per_tree = rhs._samples_per_tree;
			_attributes_per_tree = rhs._attributes_per_tree;
			_attributes_per_sample = rhs._attributes_per_sample;
