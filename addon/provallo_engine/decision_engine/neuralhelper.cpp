...
namespace provallo
{
                                   ...
                                   const size_t &ZeroPadType) : _input_dim((size_t)sqrt(input_size)), _output_dim((size_t)sqrt(outputSize)), _zeropad_type(ZeroPadType)
    {
        ...
        {
            if ((_output_dim - _input_dim) / (_input_dim + 1) != (int)(_output_dim - _input_dim) / (_input_dim + 1))
    ...
    {
        return (static_cast<int>(_weight.rows()));
    }
    ...
    void convolution_operator::operator()(int id)
    {
        ...
        int filtreDimension = sqrt(_filter_matrix.cols());
        type_matrix filtreConv = type_matrix::Zero(input.cols(), (inputDimension - filtreDimension + 1) * (inputDimension - filtreDimension + 1));
        for (int k = 0; k < inputDimension - filtreDimension + 1; k++) // For each vertical offset of the filter

        {
            for (int l = 0; l < inputDimension - filtreDimension + 1; l++) // For each lateral offset of the filter...
            {
                for (int i = 0; i < filtreDimension; i++) // Filter's Line path...
                {
                    for (int j = 0; j < filtreDimension; j++) // traverse columns
                    {
    ...
    convolution_layer::convolution_layer(size_t tailleImg, size_t nbChannels, const std::vector<type_matrix> &weight, std::function<float(float)> activationF)
        : _dimension_input((int)sqrt(tailleImg)), _weight_matrix(weight), mBias(type_matrix::Random(1, weight.size())) // ligne
          ,
    ...
    type_matrix convolution_layer::process_layer(const type_matrix &inputs)
    {
        ...
        if (inputs.cols() != _input_dimension * _input_dimension)
            throw;
        for (int n = 0; n < _weight_matrix.size(); n++) // Pour chaque filtre...
        {
    ...
    type_matrix convolution_layer::layer_backpropagation(const type_matrix &xnPartialDerivative, float step)
    {
        ...
        int weightDimension = sqrt(_weight_matrix[0].cols());
    ...
    learning_task::learning_task() : _configuration()
    {
        ...
        try
        {
            ...
            for (unsigned int i(0); i < _configuration._GenTest; i++)
            {
            ...
            else
            {
                ...
                for (int i(0); i < _configuration.genLayerSizes.size() - 1; i++)
                {
                ...
                for (int i(0); i < _configuration.disLayerSizes.size() - 1; i++)
